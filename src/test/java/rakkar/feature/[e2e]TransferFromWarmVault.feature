...
@ignore
  ...
  # 6.Submit transfer
   ...
   # 9.View transfer detail after complete
    * def getTransactionDetail = call read('Transaction.feature@View_transaction_detail_common')
    * def sourceAdress_from_sourceTransfer = response.data.sourceAddress
    * def destinationAdress_from_sourceTransfer = response.data.destinationAddress
   # --- Verify sourceName, destinationName
    * match sourceName_warm == getTransactionDetail.response.data.sourceName
  ...
  Scenario: Transfer WARM to WARM - SAME workspace
  # 1.Select token for doing transfer
    * def getToken = call read('Transfer.feature@Get_asset_transfer')
    * def tokenId_transfer = getToken.response.data.tokens[0].id
    * def tokenSymbol = getToken.response.data.tokens[0].externalAssetId
  ...
  # 3.Select destination from internal.
    ...
    * def getSource = call read('Vault.feature@SearchVaultForTransfer')
    * def destinationId_warm = get[0] getSource.response.data.vaults[?(@.type=='HOT_WALLET')].id
    * def destinationName_warm = get[0] getSource.response.data.vaults[?(@.type=='HOT_WALLET')].name
  # Get total amount of destination token before doing transfer
    * def destinationAmountBefore = get[0] getSource.response.data.vaults[?(@.type=='HOT_WALLET')].wallets[0].total
    * def walletId_destination = get[0] getSource.response.data.vaults[?(@.type=='HOT_WALLET')].wallets[0].id
  ...
  # 6.Submit transfer
    ...
    * match total_destination_after_transfer == destinationAmountBefore + amount_recieve
