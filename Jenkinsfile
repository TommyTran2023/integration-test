        ...
        stage ('Test Execution') {
            steps {
                script {
                    ...
                    env.COMMAND = "mvn clean test -Dkarate.env=${KARATE_ENV} -Dkarate.options='--tags ${tag}' -D userName='${USERNAME}' -D pass='${PASSWORD}' -D dbName='${DBNAME}' -D rerun='true' -D secret='./auto_secret.json'"
