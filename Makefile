...
COMMAND = "export SBT_HOME=/usr/src/.sbt/sbt && export MAVEN_OPTS='-Dmaven.repo.local=/usr/src/.m2/repository' && mvn clean test -Dkarate.env=dev -Dkarate.options='--tags @Get_balance_by_vaultType' -DuserName='dev_readwrite_gcp' -Dpass='TvEQwY3ZGbUf' -DdbName='dev_core_svc' -Drerun='true'"
