        ...
        stage ('Test Execution') {
            steps {
                script {
                    ...
                    env.COMMAND = "mvn clean test -Dkarate.env=${KARATE_ENV} -Dkarate.options='--tags @DisconnectWhenInitiatorGotDemoted' -D rerun='true' -D secret='./auto_secret.json' -D dbConfig='./rakkar-db-credentials.json'"
