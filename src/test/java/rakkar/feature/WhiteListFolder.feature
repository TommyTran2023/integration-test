...
Feature: WhiteList Folder
  ...
  Scenario: Check view detail a folder
    ...
    @RAKCON-36951 @ValidateDuplicateAddressAndMemo
  Scenario: Validate duplicated address and memo
    * call read(connectDB + 'Selectduplicatedwhitelistinformation') {assetExternalId: 'XRP', needTag: true}
    * def whitelist = result[0]
    * def data =
    """
    {
      "tag" : '#(whitelist.tag)',
      "address" : '#(whitelist.address)',
      "nativeAsset" : '#(whitelist.assetExternalId)',
      "externalAssetId" : '#(whitelist.assetExternalId)'
    }
    """
    ...
    * assert response.data.duplicated.length > 0
    And match each response.data.duplicated contains {id: '#(whitelist.id.toString())', name: '#(whitelist.name)', type: '#(whitelist.type)'}

    @RAKCON-36952 @ValidateDuplicatedAddressAndDifferentMemo
    Scenario: Validate duplicated address and different memo
    * call read(connectDB + 'Selectduplicatedwhitelistinformation') {assetExternalId: 'XRP'}
    * print result
    * def whitelist = result[0]
    * def data =
    """
    ...
    * assert response.data.duplicated.length == 0
    ...
    @RAKCON-36988 @ValidateDuplicatedAddressAndNoMemo
    Scenario: Validate duplicated address and no memo
    * call read(connectDB + 'Selectduplicatedwhitelistinformation') {assetExternalId: 'ETH_TEST5'}
    * print result
    * def whitelist = result[0]
    * def data =
    """
    ...
    * assert response.data.duplicated.length > 0
    ...
    @RAKCON-36989 @ValidateNoDuplicatedAddressAndMemo
    Scenario: Validate no duplicated address and memo
    * def data =
    """
    {
      "tag" : '781039150',
      "address" : 'rGE3jQ1djh7rudK3Q7mWLVVftfeFHSH3yd',
