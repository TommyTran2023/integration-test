...
# Open API from another customer
    ...
    Scenario: User unable to see list vaults from other customer
        ...
        * call read(connectDB + 'SelectVaultsOfCustomer') {customerId: #(crossTenant.accountId), vaultIds: #(actualVaultId)}
    ...
    Scenario: User able to see balance of customer by vault type and asset id
        ...
        * call read(connectDB + 'SelectBalanceOfCustomer') {customerId: #(crossTenant.accountId), type: 'COLD_WALLET', assetId: #(assetId)}
        * def data =
        """
        { 
            limit: 100, 
